...
async fn upsert_on_duplicate(api: &mut dyn TestApi) -> crate::Result<()> {
    let table = api.create_temp_table("id int primary key, x int").await?;

    let update = Update::table(&table).set("x", 2);
...
/*  NEXT TODO:
 * CREATE MYSQL UPSERT FUNC
 * Move the `pairs = update.columns.into_iter() into common code
*/
