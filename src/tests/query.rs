...
async fn json_gt_gte_lt_lte_fun(api: &mut dyn TestApi) -> crate::Result<()> {
    ...
    let select = Select::from_table(&table).so_that(path.clone().json_greater_than(Value::json(serde_json::json!(1))));
    let res = api.conn().select(select).await?;
    assert_eq!(
    ...
    let select = Select::from_table(&table).so_that(
        path.clone()
            .json_greater_than_or_equals(Value::json(serde_json::json!(1))),
    );
    let res = api.conn().select(select).await?;
    assert_eq!(
    ...
    let select = Select::from_table(&table).so_that(path.clone().json_less_than(Value::json(serde_json::json!(100))));
    let res = api.conn().select(select).await?;
    assert_eq!(
        Some(&serde_json::json!({ "a": { "b": 1 } })),
        res.get(0).unwrap()["obj"].as_json()
    ...
    let select = Select::from_table(&table).so_that(
        path.clone()
            .json_less_than_or_equals(Value::json(serde_json::json!(100))),
    );
    let res = api.conn().select(select).await?;
    assert_eq!(
        Some(&serde_json::json!({ "a": { "b": 1 } })),
