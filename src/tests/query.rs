...
async fn xml_filtering_works(api: &mut dyn TestApi) -> crate::Result<()> {
    ...
    let three = Insert::single_into(&table).value("xmlField", Value::Xml(None));

    api.conn().insert(one.into()).await?;
    api.conn().insert(two.into()).await?;
    api.conn().insert(three.into()).await?;
    ...
    // Not equals
    {
        let select = Select::from_table(&table).so_that(Column::from("xmlField").not_equals(Value::Xml(None)));
        let result = api.conn().query(select.into()).await?;
        assert_eq!(result.len(), 1);
