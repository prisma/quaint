    ...
    fn from(pv: Value<'a>) -> Self {
        let res = match pv {
            ...
            Value::Float(f) => f.map(|f| match Number::from_f64(f as f64) {
                ...
                None if f.is_nan() => serde_json::Value::String("NaN".into()),
                None if f == f32::INFINITY => serde_json::Value::String("Infinity".into()),
                None => serde_json::Value::String("-Infinity".into()),
            }),
            Value::Double(f) => f.map(|f| match Number::from_f64(f) {
                Some(number) => serde_json::Value::Number(number),
                None if f.is_nan() => serde_json::Value::String("NaN".into()),
                None if f == f64::INFINITY => serde_json::Value::String("Infinity".into()),
                None => serde_json::Value::String("-Infinity".into()),
            }),
...
impl<'a> Value<'a> {
    ...
    /// Returns a f64 if the value is a numeric value and the underlying decimal
    /// can be converted, otherwise `None`.
    pub fn as_f64(&self) -> Option<f64> {
        match self {
            Value::Double(Some(f)) => Some(*f as f64),
            _ => None,
        }
    }

    /// Returns a f32 if the value is a numeric value and the underlying decimal
    /// can be converted, otherwise `None`.
    ...
    /// `true` if the `Value` is a numeric value or can be converted to a one.
    #[cfg(feature = "bigdecimal")]
    #[cfg_attr(feature = "docs", doc(cfg(feature = "bigdecimal")))]
